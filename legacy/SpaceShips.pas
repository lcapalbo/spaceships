...
PASTILLA
  CELESTE=100p
  VERDE=COMPLETA ENERGÍA
  ROJA=CAMBIA CONTROLES
  BLANCO=VELOCIDAD
  VIOLETA=DISPARO EN ÁNGULO
  AMARILLA=LASER
  ROSA=ESCUDO}
...
V: Top5;
SEGRABO:BOOLEAN;
  {PARA COPIAR LAS IMÁGENES A MEMORIA}
...
Begin
ClearDevice;
                                 {COPIA LA IMÁGEN EN UNA UBICACIÓN DE MEMORIA}
SetBkColor(0);                                                             {I}
...
BEGIN
  ...
  if ((NOT UNOJUEGA)AND(NOT DOSJUEGA))OR(VERCUADRO) then
    begin                                                   {PASA DE PANTALLA}
     ...
     IF (NOT DOS) THEN                                {SI HAY UN SÓLO JUGADOR}
     BEGIN
...
SetColor(15);SetTextStyle(2, 1, 10);OutTextXY(430,20,'Adventure!');
      SetTextStyle(2, 0, 6);
      tipear('Edición Especial Para:',210,5,0,11,1);
...
Case t of
     ...
     chr(75),'4','a','A': begin
                          ...
                          if y=325 then
                                   ...
                                   Case velocidad of
                                     ...
                                     10: begin velocidad:=6; veloc:='Rápido' end;
                          ...
                          if y=375 then
                                   begin
                                   Case dificultad of
                                     50: begin dificultad:=100; dificul:='Fácil' end;
                                     100: begin dificultad:=4; dificul:='Difícil' end;
     ...
     chr(77),'6','d','D': begin
                          ...
                          if y=325 then
                                   begin
                                   Case velocidad of
                                     8: begin velocidad:=6; veloc:='Rápido' end;
                          ...
                          if y=375 then
                                   begin
                                   Case dificultad of
                                     50: begin dificultad:=4; dificul:='Difícil' end;
                                     100: begin dificultad:=50; dificul:='Media' end;
                                     4: begin dificultad:=100; dificul:='Fácil' end
                                   end;
                                   SetFillStyle(1,1); Bar(400,350,550,400);
                                   end;
...
SetFillStyle(1,1); FloodFill(403,3,12);
                           {UBICACION DEL TÍTULO, SEGUN CANTIDAD DE JUGADORES}
IF NOT DOS THEN
 BEGIN
 SetTextStyle(4, 0, 8);
...
tipear('Energía',569,104,0,4,1);tipear('Energía',570,105,0,12,1);
...
IF DOS THEN
       ...
       tipear('Energía',569,414,0,4,1);tipear('Energía',570,415,0,12,1);
...
DibujarJugador(x,y,1);
                               {COPIA LA IMÁGEN EN UNA UBICACIÓN DE MEMORIA P}
...
IF DOS THEN
   ...
   MostrarEnergia(energia1,energiaINICIAL,2);
                               {COPIA LA IMÁGEN EN UNA UBICACIÓN DE MEMORIA Q}
